...
- 整個正方形畫布背景必須為純黑色（R=0, G=0, B=0, #000000），四個角落及所有背景區域均填滿純黑，不得有任何漸層或雜色
- 角色（人物、裝飾）的所有輪廓外圍必須繪製 4–6px 的純白色（#FFFFFF）粗外框描邊，包覆頭髮、耳朵、手臂、腳等所有邊緣
- 白色描邊與黑色背景之間不得有漸層或半透明過渡，必須是清晰的硬邊界
...
【裝飾】在角色周圍點綴 2–4 個小閃光或星星（集中在畫布中央區域，裝飾也需加白色描邊）

【輸出】單一正方形 PNG，背景純黑 #000000，角色有白色外框描邊。
...
【畫布規格 — Chroma Key 去背模式】
- 整個正方形畫布背景必須為純黑色（R=0, G=0, B=0, #000000），四個角落及所有背景區域均填滿純黑
- 角色（人物、裝飾）的所有輪廓外圍必須繪製 4–6px 的純白色（#FFFFFF）粗外框描邊，包覆所有邊緣
- 白色描邊與黑色背景之間不得有漸層或半透明過渡，必須是清晰的硬邊界
