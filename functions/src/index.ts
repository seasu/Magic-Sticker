...
const STYLE_CHAR_DESC: string[] = [
  ...
  "根據照片人物繪製「ゆるい（鬆散可愛）」塗鴉風格的完整 Q 版角色（頭頂至腳底完整呈現）\n" +
    ...
    "  * 舉臂揮手等動作時，手臂末端距上緣 ≥ 25%；角色整體邊界（含姿勢極端點）高度 ≤ 畫布 45%",
  // 10 showaManga
...
function buildPrompt(p: PromptParams): string {
  ...
  const commonCharSection =
    `⚠️【構圖留白 — 最高優先，不得違反，任何風格設定均服從此規則】\n` +
    `角色所有部位（頭頂、舉起的手掌尖端、伸展的手臂末端、腳底、裝飾物）：\n` +
    `  · 距上緣留白 ≥ 25%（含舉高的手臂末端；此規則優先於所有風格效果與構圖慣例）\n` +
    `  · 距下緣留白 ≥ 15%\n` +
    `  · 距左緣 ≥ 10%、距右緣 ≥ 10%\n` +
    `  · 角色整體垂直高度（從最高部位到最低部位，含動態姿勢極端點）≤ 畫布 45%\n` +
    `  · 嚴禁任何部位被截斷\n` +
