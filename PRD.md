...
| `generateStickerImageV2` | 1 GiB | 200s | AI 圖片生成 V2（新版 App ≥ v3.18.30；純 metadata 協議，CF 端組 prompt；v1.3.0 起新增頂部截斷自動偵測 & retry）|
...
| 版本 | 日期 | 摘要 |
| v3.18.47 | 2026-07-22 | **chore(deps): 升級 Google Play Billing Library 至 8.0.0（因應 Google Play 2026/8/31 強制下限）**：① `pubspec.yaml` `in_app_purchase` 從 `^3.2.0` 升級為 `^3.3.0`，帶動 transitive 依賴 `in_app_purchase_android` 從 `0.4.0+5` 升級為 `0.5.2`（內含 Google Play Billing Library 8.0.0，符合 Google Play Console 要求的 8.0.0+ 下限）；② `environment.sdk` 下限由 `>=3.7.0` 提高為 `>=3.10.0`（配合 `in_app_purchase` 3.3.0 的最低 SDK 需求）；③ `.github/workflows/main_build.yml`、`.github/workflows/pr_check.yml` 的 `flutter-version` 從 `3.29.1` 升級為 `3.44.7`（stable，內含 Dart 3.12.2），確保 CI 能解析新依賴版本；④ `dart analyze --fatal-infos` 通過，`IAPService`（`lib/features/billing/services/iap_service.dart`）未直接呼叫任何 v8 移除的 API（`queryPurchaseHistory` 等），全部經由 `in_app_purchase` 抽象層呼叫，無需修改業務邏輯程式碼。 |
