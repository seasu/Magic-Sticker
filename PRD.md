...
| 版本 | 日期 | 摘要 |
|---|---|---|
| v3.13.9 | 2026-03-25 | **feat(cd-ios-firebase-dist)**：`main_build.yml` `ios-build` job 新增「Upload iOS to Firebase App Distribution」步驟（`wzieba/Firebase-Distribution-Github-Action@v1`），緊接 TestFlight 上傳之後執行；使用 `FIREBASE_APP_ID_IOS` secret 及 `testers` 群組，release notes 邏輯與 Android 一致。需新增 GitHub Secret：`FIREBASE_APP_ID_IOS`。 |
