...
| 版本 | 日期 | 摘要 |
|---|---|---|
| v3.13.43 | 2026-03-26 | **refactor(ci-remove-python-pillow)**：完全移除 CI icon 生成步驟對 Python/Pillow 的依賴，新增 `tool/generate_small_app_icon.dart`（使用 Dart `image` 套件）統一產生 `assets/images/app_icon_small.png`。`main_build.yml` Android / iOS 兩處改為 `dart run tool/generate_small_app_icon.dart`，避免 `PIL` 缺失與 PEP 668 `externally-managed-environment` 問題。 |
